    ...
    context 'ログイン済み' do
      ...
      context '同じ条件のリレーションが存在しない' do
        ...
        it 'データベースにフレンドの新しいリレーションが登録される' do
          expect do
            post :create, params: { user_id: @friend.id }
          end.to change(@friend.relationships, :count).by(1)
        end
      ...
      context '既に同じ条件のリレーションが存在する' do
        ...
        it 'データベースにユーザーの新しいリレーションが登録されない' do
          expect do
            post :create, params: { user_id: @friend.id }
          end.not_to change(@user.relationships, :count)
        end
        it 'データベースにフレンドの新しいリレーションが登録されない' do
          expect do
            post :create, params: { user_id: @friend.id }
          end.not_to change(@friend.relationships, :count)
        end
        it '友達ページにリダイレクトする' do
          post :create, params: { user_id: @friend.id }
          expect(response).to redirect_to friends_users_url
  ...
  describe 'Patch #update' do
    ...
    context 'ログイン済み' do
      ...
      it 'リレーションstatusがapprovedになる' do
        expect([@user_relationship.reload.status, @friend_relationship.reload.status]).to eq %w[approved approved]
      end
      it '友達ページにリダイレクトする' do
        expect(response).to redirect_to friends_users_url
      end
